...
- **Dependency policy (pnpm 11):** pnpm's default minimum release age applies (packages must be ≥24h old to install) with **no exclusions** by design — don't add `minimumReleaseAgeExclude`. Dependency build scripts are denied; the `allowBuilds` map in `pnpm-workspace.yaml` lists reviewed packages, all intentionally `false` (they ship prebuilt binaries). Flip an entry to `true` only after reviewing its install script.
